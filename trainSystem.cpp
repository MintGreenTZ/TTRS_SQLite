...
std::pair<int, pqxx::result> trainSystem::getTrainInfo(std::string trainID) {
	std::ostringstream q;
...
trainSystem::trainSystem(database *_c, std::string _tableName, querySystem *_query) :
...
std::pair<int, std::string> trainSystem::query_train(std::string trainID, std::string date) {
...
int trainSystem::delete_train(std::string trainID) {
	auto info = getTrainInfo(trainID);
	if (info.first == -1 || info.second[0][corres["released"]].as<bool>() == true) return -1;
	std::ostringstream q;
...
std::pair<std::string, std::string> trainSystem::findTime(std::string trainID, std::string date, std::string FROM, std::string TO) {
	auto info = getTrainInfo(trainID);
	std::vector<std::string> stations = arrayParser<std::string>::parse(info.second[0][corres["stations"]].as<std::string>());
	std::vector<int> travelTimes = arrayParser<int>::parse(info.second[0][corres["travelTimes"]].as<std::string>());
	std::vector<int> stopoverTimes = arrayParser<int>::parse(info.second[0][corres["stopoverTimes"]].as<std::string>());
